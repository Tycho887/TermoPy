...
class Test_case:
    ...
    diatomic = not monatomic

class Test_TermoPy_processes(unittest.TestCase):

    def test_isothermal_compression_data_from_dP(self):
        print("\nRunning isothermal process data from dP test\n")
        for i in range(num_tests):
            ...
            Work = process.work_done_on

            # we want to calculate the expected work done on the gas
            self.assertTrue(abs(Work - n * tp.R * T * np.log(V/process.volume[0])) < allowed_error)
            self.assertTrue(abs(Q_in - Work) < allowed_error)
    ...
    def test_isothermal_expansion_data_from_dP(self):
        print("\nRunning isothermal process data from dP test\n")
        for i in range(num_tests):
            case = Test_case()
            P, V, T, n = case.P, case.V, case.T, case.n
    ...
    def test_isothermal_compression_data_from_dV(self):
        print("\nRunning isothermal process data from dV test\n")
        for i in range(num_tests):
            case = Test_case()
            P, V, T, n = case.P, case.V, case.T, case.n
            ...
            # we want to calculate the expected work done on the gas
            self.assertTrue(abs(Work - n * tp.R * T * np.log(process.volume/process.volume[0])) < allowed_error)
            self.assertTrue(abs(Q_in - Work) < allowed_error)
            self.assertTrue(process.is_ideal_gas())
            self.assertTrue(process.is_first_law_satisfied())
    ...
    def test_isothermal_expansion_data_from_dV(self):   
        print("\nRunning isothermal process data from dV test\n")
        for i in range(num_tests):
            case = Test_case()
            P, V, T, n = case.P, case.V, case.T, case.n
            ...
            process.generate_data_from_dV(V*20)
